...
grep -Fq 'read -r _setup_import_answer < /dev/tty' "$ROOT/setup.sh" || fail "simple setup must offer interactive config import through the controlling terminal"
grep -Fq 'retry_download "$PROJECT_RAW_BASE/config-import.sh" "$CONFIG_IMPORT_STAGE"' "$ROOT/setup.sh" || fail "simple setup must fetch the importer only after the user chooses to continue"
grep -Fq 'sh "$CONFIG_IMPORT_STAGE"' "$ROOT/setup.sh" || fail "simple setup must continue directly into the safe importer"
grep -Fq 's|S|skip|SKIP)' "$ROOT/setup.sh" || fail "simple setup must allow the user to skip config import"
