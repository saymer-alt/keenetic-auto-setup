...
grep -q '^NETFILTER_COMPONENT_ID=opkg-kmod-netfiltergrep -q '^EXT_COMPONENT_ID=ext$' "$ROOT/install.sh" || fail "installer must use KeeneticOS Ext filesystem component id"
...
grep -q '^DNS_TLS_COMPONENT_ID=dns-tlsgrep -q '^EXT_COMPONENT_ID=ext$' "$ROOT/install.sh" || fail "installer must use KeeneticOS Ext filesystem component id"
grep -q '^EXT_UTILS_COMPONENT_ID=ext-utils$' "$ROOT/install.sh" || fail "installer must use KeeneticOS EXT4 utilities component id"
grep -q 'Checking required KeeneticOS components' "$ROOT/install.sh" || fail "installer must run named-component preflight"
grep -q 'Missing required KeeneticOS component(s):' "$ROOT/install.sh" || fail "installer must label the missing-component list"
grep -Fq 'printf '\''%s\n'\'' "${_rc_missing_lines#?}" >&2' "$ROOT/install.sh" || fail "missing-component list must not start with a blank line"
...
pass "RU/EN HOWTOs mirror storage-mode, external EXT4 and ProxyN contracts"
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS DNS-over-TLS component id"
grep -q '^DNS_HTTPS_COMPONENT_ID=dns-httpsgrep -q '^EXT_COMPONENT_ID=ext$' "$ROOT/install.sh" || fail "installer must use KeeneticOS Ext filesystem component id"
grep -q '^EXT_UTILS_COMPONENT_ID=ext-utils$' "$ROOT/install.sh" || fail "installer must use KeeneticOS EXT4 utilities component id"
grep -q 'Checking required KeeneticOS components' "$ROOT/install.sh" || fail "installer must run named-component preflight"
grep -q 'Missing required KeeneticOS component(s):' "$ROOT/install.sh" || fail "installer must label the missing-component list"
grep -Fq 'printf '\''%s\n'\'' "${_rc_missing_lines#?}" >&2' "$ROOT/install.sh" || fail "missing-component list must not start with a blank line"
...
pass "RU/EN HOWTOs mirror storage-mode, external EXT4 and ProxyN contracts"
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS DNS-over-HTTPS component id"
grep -Fq 'if ! component_list_has "$DNS_TLS_COMPONENT_ID" && ! component_list_has "$DNS_HTTPS_COMPONENT_ID"; then' "$ROOT/install.sh" || fail "installer must require at least one secure-DNS component, not both"
