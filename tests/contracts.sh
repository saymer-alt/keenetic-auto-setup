...
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS component id proxy for Proxy client"
...
_proxy_preflight_line=$(grep -n '^require_proxy_client_component
...
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" | head -1 | cut -d: -f1)
_opkg_update_line=$(grep -n '^log "Updating opkg\.\.\."
