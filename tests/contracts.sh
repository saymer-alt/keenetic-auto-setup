...
grep -q '^NETFILTER_COMPONENT_ID=opkg-kmod-netfiltergrep -q 'Checking required KeeneticOS components' "$ROOT/install.sh" || fail "installer must run named-component preflight"
...
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS component id ext for external storage"
grep -q '^EXT_UTILS_COMPONENT_ID=ext-utils
for _f in install.sh mihomo-doctor.sh update-mihomo.sh; do
    grep -q '^MIHOMO_STAGE_MARGIN_KB=4096$' "$ROOT/$_f" || fail "$_f must use the shared 4 MB Mihomo staging margin"
done
! grep -q '32768' "$ROOT/install.sh" || fail "installer must not use the old fixed 32 MB free-space threshold"
...
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS component id ext-utils for external storage"
...
grep -Fq 'if [ "$OPT_CLASS" = "external" ] && ! component_list_has "$EXT_COMPONENT_ID"; then' "$ROOT/install.sh" || fail "external /opt must require the ext component"
grep -Fq 'if [ "$OPT_CLASS" = "external" ] && ! component_list_has "$EXT_UTILS_COMPONENT_ID"; then' "$ROOT/install.sh" || fail "external /opt must require the ext-utils component"
grep -Fq 'Unsupported external /opt filesystem:' "$ROOT/mihomo-doctor.sh" || fail "Doctor must FAIL unsupported external /opt filesystems"
grep -Fq 'Required external-storage KeeneticOS component missing: ext' "$ROOT/mihomo-doctor.sh" || fail "Doctor must diagnose missing ext/ext-utils components"
grep -Fq 'UNSUPPORTED EXTERNAL /opt FILESYSTEM:' "$ROOT/update-mihomo.sh" || fail "updater must warn on legacy non-EXT4 external /opt"
pass "external Entware storage contract is EXT4-only with ext + ext-utils component checks"
...
pass "RU/EN HOWTOs mirror storage-mode and ProxyN contracts"
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS Netfilter component id"
grep -q '^EXT_COMPONENT_ID=extgrep -q 'Checking required KeeneticOS components' "$ROOT/install.sh" || fail "installer must run named-component preflight"
grep -q 'Missing required KeeneticOS component(s):' "$ROOT/install.sh" || fail "installer must label the missing-component list"
grep -Fq 'printf '\''%s\n'\'' "${_rc_missing_lines#?}" >&2' "$ROOT/install.sh" || fail "missing-component list must not start with a blank line"
grep -q 'Proxy client / Клиент прокси (${PROXY_COMPONENT_ID})' "$ROOT/install.sh" || fail "installer must name missing Proxy client clearly"
grep -q 'Cloud-based content filtering and ad blocking / Фильтрация контента и блокировка рекламы при помощи облачных сервисов (${DNS_FILTER_COMPONENT_ID})' "$ROOT/install.sh" || fail "installer must name missing dns-filter clearly"
...
pass "RU/EN HOWTOs mirror storage-mode and ProxyN contracts"
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS Ext filesystem component id"
grep -q '^EXT_UTILS_COMPONENT_ID=ext-utilsgrep -q 'Checking required KeeneticOS components' "$ROOT/install.sh" || fail "installer must run named-component preflight"
grep -q 'Missing required KeeneticOS component(s):' "$ROOT/install.sh" || fail "installer must label the missing-component list"
grep -Fq 'printf '\''%s\n'\'' "${_rc_missing_lines#?}" >&2' "$ROOT/install.sh" || fail "missing-component list must not start with a blank line"
grep -q 'Proxy client / Клиент прокси (${PROXY_COMPONENT_ID})' "$ROOT/install.sh" || fail "installer must name missing Proxy client clearly"
grep -q 'Cloud-based content filtering and ad blocking / Фильтрация контента и блокировка рекламы при помощи облачных сервисов (${DNS_FILTER_COMPONENT_ID})' "$ROOT/install.sh" || fail "installer must name missing dns-filter clearly"
...
pass "RU/EN HOWTOs mirror storage-mode and ProxyN contracts"
echo "[OK] Contract smoke tests passed"
 "$ROOT/install.sh" || fail "installer must use KeeneticOS EXT4 utilities component id"
grep -q 'Checking required KeeneticOS components' "$ROOT/install.sh" || fail "installer must run named-component preflight"
grep -q 'Missing required KeeneticOS component(s):' "$ROOT/install.sh" || fail "installer must label the missing-component list"
grep -Fq 'printf '\''%s\n'\'' "${_rc_missing_lines#?}" >&2' "$ROOT/install.sh" || fail "missing-component list must not start with a blank line"
grep -q 'Proxy client / Клиент прокси (${PROXY_COMPONENT_ID})' "$ROOT/install.sh" || fail "installer must name missing Proxy client clearly"
