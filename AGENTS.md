...
| Скрипт                    | Роль |
|---------------------------|------|
| install.sh                | основной установщик (ARM; режимы `ram`\|`disk`, ram = tmpfs) |
| install_7621.sh           | legacy для MT7621: сломанный TLS → `--insecure`, http-зеркало, пин ipk |
...
- Архитектуры: aarch64 (основная); mipsel — только install_7621.sh; суффиксы ipk:
...
- README.md (RU+EN) — входная точка проекта: EN-версия основная и первая, ниже полная
  русская; содержит концептуальное вступление, архитектуру, use cases, экосистему
...
- docs/00–10 — подробные гайды;
  ARCHITECTURE.md — философия слоёв; CHANGELOG.md — изменения (теги v1.0.0, v1.1.0).
...
- install_7621.sh существует из-за сломанного TLS на MT7621: `--insecure` + http-зеркало
