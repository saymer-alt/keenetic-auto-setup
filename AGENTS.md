...
- RAM/swap policy: 256 MB+ is the supported profile (live-tested; internal-storage operation proven), 512 MB+ has the most headroom; swap is optional on both (256 MB: extra headroom note, never required). 128 MB-class is best-effort/experimental and REQUIRES active swap (SwapTotal) >= 384 MB (512 MB preferred): install.sh stops at the early preflight, before downloads/mutations, without it; with it, a prominent EXPERIMENTAL warning stays; stability is not guaranteed even with swap; prefer `disk` mode; never weaken the one-Mihomo rule. The project never creates/resizes swap itself. KeeneticOS zRAM (compressed swap in RAM, no NAND swap file; recommended for internal-storage installs, UI path wording varies by firmware) is auto-sized to about the physical RAM, so on the 128 MB class it does not satisfy the prerequisite alone - storage-backed swap may coexist; the Doctor classifies backends from /proc/swaps (zRAM vs storage-backed) and everything is judged from total ACTIVE swap; verify with mihomo-doctor.sh. Low-RAM instability remains a known production risk (docs/06).
