...
#
# Usage:
#   mihomo-route-watch.sh [-g GROUP] [-u URL] [-s SECRET]
#                         [--watch N]
#
...
command -v curl >/dev/null 2>&1 || {
    echo "[route-watch] curl is required but not installed" >&2
    exit 1
}
command -v jq >/dev/null 2>&1 || {
    echo "[route-watch] jq is required but not installed" >&2
    exit 1
}
...
if [ -z "$WATCH" ]; then
    ...
    case "$FETCH_STATE" in
        ...
        notfound)
            ...
            exit 4 ;;
        http)
            echo "[route-watch] unexpected HTTP status from the controller: $CODE" >&2
            exit 2 ;;
        badjson)
            echo "[route-watch] controller returned malformed JSON" >&2
            exit 5 ;;
    esac
    resolve "$GROUP"
