...
- RAM и выбор /opt-памяти — раздельные решения:
  - **256 МБ+ RAM — поддерживаемый профиль** (проверен вживую). **512 МБ+ — самый запасной профиль**, swap опционален. 256 МБ с /opt на внутренней памяти Keenetic требует **активного штатного zRAM** («Сжатый RAM-диск для системного swap»; без него установщик останавливается на раннем preflight); 256 МБ с внешним /opt — swap/zRAM опциональны.
  - **128 МБ-класс — только best-effort/experimental и только при выполнении ВСЕХ условий**: (1) /opt на **внешнем** постоянном носителе; (2) активный **storage-backed swap на внешнем носителе**; (3) его объём **>= 384 МБ** (512 МБ предпочтительно). Без любого из условий установщик останавливается на раннем preflight, до скачиваний и изменений. **zRAM не считается**: авто-размер KeeneticOS zRAM примерно равен физической RAM (~128 МБ), он может сосуществовать с дисковым swap, но не заменяет его. Swap и носители проект сам никогда не создаёт, не монтирует и не меняет. Стабильность не гарантирована даже со swap.
