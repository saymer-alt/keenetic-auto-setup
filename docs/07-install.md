...
- `disk` — persistent `/opt`, `S00ubifs` не ставится. Для внешнего `/opt` поддерживается только EXT4.
...
Новый канонический Proxy-интерфейс имеет description `mihomo t2sN`, protocol
SOCKS5, включённый `socks5-udp` и upstream `127.0.0.1:7890`.

Для обратной совместимости ownership старой установки определяется не по имени, а
по функциональной сигнатуре: **SOCKS5 + socks5-udp + 127.0.0.1:7890**. Поэтому
старые description вроде `mihomo` или platform/default naming не являются WARN/FAIL:
такой ProxyN переиспользуется без переименования. Порядок выбора —
канонический → legacy-compatible → первый свободный `ProxyN`. Чужой Proxy без
полной функциональной сигнатуры не перезаписывается.
