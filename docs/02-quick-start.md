...
* **256+ MB RAM — поддерживаемый профиль.** На 256 МБ нужен один активный backend: штатный zRAM **или** внешний storage-backed swap; место /opt на этот выбор не влияет. zRAM и дисковый swap одновременно по рекомендации производителя не используем. На 512 МБ+ swap/zRAM опциональны. 128 MB-класс допускается только как best-effort/experimental при внешнем /opt + внешнем storage-backed swap >=384 МБ (project-specific floor; zRAM не считается)
