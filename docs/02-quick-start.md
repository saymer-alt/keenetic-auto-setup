...
* **256+ MB RAM — поддерживаемый профиль** (512 МБ+ — самый запасной; active swap не обязателен, на 256 МБ даёт дополнительный запас). 128 MB-класс допускается только как best-effort/experimental **и только с активным swap от 384 МБ** (512 МБ предпочтительно; без достаточного swap установщик останавливается на раннем preflight; предпочтительнее `disk`)
