...
- [x] ранняя RAM/swap/носитель-проверка: 128MB-класс — только внешний /opt + внешний storage-backed swap >= 384 МБ (512 предпочтительно; zRAM не считается), иначе остановка до изменений; 256 МБ — внутренняя память только с активным zRAM, внешний /opt без жёсткого требования; 512 МБ+ — swap опционален
