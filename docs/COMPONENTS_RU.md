...
| KeeneticOS **Модули ядра подсистемы Netfilter** (`opkg-kmod-netfilter`) | обязательный компонент для штатного VoIP bypass | предоставляет `xt_multiport`, без которого `020-bypass-wa.sh` не может установить свои UDP multiport MARK/CONNMARK/RETURN правила; A/B/C-тест KN-1010/5.1.6 это воспроизвёл после reboot | ранний read-only preflight через `show version`; Doctor дополнительно проверяет фактические `_CUST_BYPASS_WA_` rules |
