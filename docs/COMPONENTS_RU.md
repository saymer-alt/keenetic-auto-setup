...
> **Doctor и installer намеренно различаются для legacy-систем.** Installer новой/повторно провиженной поддерживаемой установки требует component IDs заранее. Doctor оценивает уже работающий роутер: если `dns-filter` или `opkg-kmod-netfilter` не отображается в `show version`, но соответствующая runtime-capability доказанно жива, это WARN о дрейфе профиля, а не утверждение о текущем отказе. Такой случай воспроизведён на KN-3811 / KeeneticOS 5.1.5.
