...
> **Важно про `show version`.** Keenetic может переносить длинные component ID внутри самого имени по ширине вывода: на KN-3811 / KeeneticOS 5.1.5 и 5.1.6 реально наблюдались `dns-` + `filter` и `opkg-kmod-` + `netfilter` на соседних строках. Это форматирование, а не отсутствие компонента. Installer и Doctor сначала нормализуют поле `components:` и только потом проверяют точные comma-delimited ID. Для действительно legacy-систем Doctor всё равно отдельно различает profile compliance и доказанную runtime-capability.
