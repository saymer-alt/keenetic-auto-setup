...
- **WARP / MASQUE and Cloudflare colo.** `NODE`/colo and `SEEN AS`/exit country are separate. WARPSCOUT can scan WARP endpoint+port combinations and, for WG/AWG, find different Cloudflare nodes; that is useful for steering the processing colo but does not guarantee a different public-IP country. MASQUE H3 (`-p masque`, QUIC/UDP) and H2 (`-p masque-h2`, TCP/TLS) use different address pools and behave differently: within one MASQUE scan the node is network-dependent rather than endpoint-selectable. See the detailed Russian guide [35-warpscout.md](encyclopedia/35-warpscout.md).
...
The current release has direct retained live evidence on **KN-1010 (MT7621AT / mipsel)** and **KN-1012, KN-3811, KN-3812, Netcraze Ultra NC-1812 (AArch64 / ARM64)**. The home 1812-class device in these tests is specifically **NC-1812**. NC-1812 and Keenetic Titan KN-1812 are different market models on the same MT7988D-class hardware platform, so an NC-1812 run must not be presented as a separate physical KN-1812 acceptance. Evidence keeps the exact `model` / `hw_id` reported by the router itself. Keenetic's references identify MT7981B AArch64 in KN-1012/3811/3812 and prescribe the `mipsel` OPKG archive for KN-1010. Earlier field use also includes **KN-1810** and **KN-1913** (see [CHANGELOG](../CHANGELOG.md)). The code and package paths also support `armv7` and big-endian `mips`, but the current repository does not record equally fresh live acceptance for those two paths.
...
7. **Selects/creates the project Proxy interface (ProxyN)**. It prefers the canonical `mihomo t2sN` profile, but recognizes older installs by the functional signature SOCKS5 + `socks5-udp` + upstream `127.0.0.1:7890` regardless of the old description. A legacy-compatible Proxy is reused without renaming; a new free `ProxyN` is created only when no compatible bridge exists.
...
9. Backs the old binary up to **`/tmp`** (RAM — not `/opt`), replaces the binary, then atomically writes `/opt/etc/keenetic-auto-setup-mihomo.state` with the committed runtime version/source/asset/package release/size. The previous state file is backed up too and is restored together with the binary on rollback.
...
- The Entware opkg database is **not rewritten by `update-mihomo.sh`**. That database describes the installed package as a whole, while this updater deliberately replaces only the canonical Mihomo binary. Manually changing the opkg `Version:` field would falsely claim that the full package payload was installed. The project-owned state file records the binary truth instead; Doctor compares runtime → project state → opkg metadata separately.
- If a Web-UI/upstream core is already the same version as `entware-go:latest`, a normal updater run skips replacement; `--force` deliberately replaces the same-version upstream-form binary with the project's packaged/UPX form and writes project binary state. When a newer `entware-go` runtime becomes available, a normal update replaces the older upstream-form binary in the same way.
