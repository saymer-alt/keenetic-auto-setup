...
> **Important `show version` detail.** Keenetic may wrap a long component ID inside the token at the output width. KN-3811 / KeeneticOS 5.1.5 and 5.1.6 reproduced `dns-` + `filter` and `opkg-kmod-` + `netfilter` on adjacent physical lines. That is formatting, not component absence. Installer and Doctor normalize only the `components:` field before exact comma-delimited matching. For genuine legacy drift, Doctor still distinguishes supported-profile compliance from independently proven runtime capability.
