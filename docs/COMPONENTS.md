...
| KeeneticOS **Cloud-based content filtering and ad blocking / Фильтрация контента и блокировка рекламы при помощи облачных сервисов** (`dns-filter`) | required KeeneticOS component for the current supported DNS-interception profile | supplies the DNS-filter/interception component family used by the mandatory `dns-proxy intercept enable` profile | installer hard-requires the component ID; on legacy systems Doctor correlates absence with live `dns-proxy intercept enable`: live capability → profile-drift WARN, absent capability → FAIL |
| KeeneticOS **Kernel modules for Netfilter / Модули ядра подсистемы Netfilter** (`opkg-kmod-netfilter`) | required KeeneticOS component for the current default VoIP bypass path | provides `xt_multiport`; without it `020-bypass-wa.sh` cannot install its UDP multiport MARK/CONNMARK/RETURN rules; a KN-1010/5.1.6 A/B/C reboot test reproduced this dependency | installer hard-requires the component ID; on legacy systems Doctor checks the live `_CUST_BYPASS_WA_` rules: live rules → profile-drift WARN, broken runtime path → FAIL |
...
> **Doctor and installer deliberately differ for legacy systems.** A new/reprovisioned supported install must pass the component-ID preflight. Doctor evaluates an already-running router: if `dns-filter` or `opkg-kmod-netfilter` is absent from `show version` but the corresponding runtime capability is proven live, Doctor reports supported-profile drift as WARN rather than claiming a current outage. KN-3811 / KeeneticOS 5.1.5 reproduced this case.
