...
| KeeneticOS **Cloud-based content filtering and ad blocking / Фильтрация контента и блокировка рекламы при помощи облачных сервисов** (`dns-filter`) | required KeeneticOS component for the current supported DNS-interception profile | supplies the DNS-filter/interception component family used by the mandatory `dns-proxy intercept enable` profile | installer hard-requires the component ID after normalizing wrapped `show version` output; on legacy systems Doctor additionally correlates genuine absence with live `dns-proxy intercept enable`: live capability → profile-drift WARN, absent capability → FAIL |
| KeeneticOS **Kernel modules for Netfilter / Модули ядра подсистемы Netfilter** (`opkg-kmod-netfilter`) | required KeeneticOS component for the current default VoIP bypass path | provides `xt_multiport`; without it `020-bypass-wa.sh` cannot install its UDP multiport MARK/CONNMARK/RETURN rules; a KN-1010/5.1.6 A/B/C reboot test reproduced this dependency | installer hard-requires the component ID after normalizing wrapped `show version` output; on legacy systems Doctor additionally checks the live `_CUST_BYPASS_WA_` rules: live rules → profile-drift WARN, broken runtime path → FAIL |
...
> **Important parsing rule for `show version`.** Human-readable CLI output is not a stable line-oriented API. An earlier capture from the operator's home Netcraze Ultra NC-1812 / KeeneticOS 5.1.5 on 2026-09-19 already showed this formatting class: a component ID could be split inside the token across physical lines (for example `ike-` + `client`). That was treated as harmless wrapping and no regression was created — an early signal we missed. On 2026-09-25 KN-3811 / 5.1.5 and 5.1.6 exposed the behavioral impact: `dns-` + `filter` and `opkg-kmod-` + `netfilter` produced false “missing” evidence under line-oriented matching. Installer and Doctor now normalize **only the logical `components:` field**, stop at the next field, then exact-match comma-delimited IDs. The permanent regression splits every required component ID at every possible internal position. Do not generalize this technique to `show running-config`, `/proc/mounts`, `/proc/swaps`, or `opkg list-installed`: there a physical line is a semantic command/record. Incomplete or unconvincing output must become UNKNOWN/UNVERIFIED, not evidence of absence. For genuine legacy drift, Doctor still distinguishes supported-profile compliance from independently proven runtime capability.
