...
- **NC-1012 Mihomo file provenance:** the external-EXT4 NC-1012 currently shows a canonical `/opt/sbin/mihomo` around **54.6 MB**, while the internal-UBIFS KN-1012 shows about **13 MB** with the same runtime version 1.19.31. The current `saymer-alt/entware-go` Mihomo build recipe explicitly packs the package binary with `upx -9 --lzma`, so the smaller KN-1012 file is consistent with the current packaged form. The exact origin/form of the larger NC-1012 binary is not proven by the retained transcript. Both routers are running correctly; treat the difference as a read-only provenance/form question, not evidence of corruption. If investigated, record hashes/file metadata/package source before changing anything.
