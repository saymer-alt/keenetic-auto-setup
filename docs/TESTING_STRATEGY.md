...
Both failures were converted into permanent checks/regressions. This is the model to follow: a real failure should leave behind a small test that prevents that exact failure from returning.
...
3. **Existing permanent regression tests.** Extend the current harness with the smallest scenario that represents the bug.
...
A release does not require rerunning every historical synthetic campaign. The gate should instead confirm:
