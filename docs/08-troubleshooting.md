...
Если 128MB — это best-effort профиль: проверь свободную RAM, не запускай второй Mihomo, а при нестабильности перейди с `ram` на `disk`/отключи S00ubifs. Для штатной эксплуатации рекомендуется 256+ MB.
