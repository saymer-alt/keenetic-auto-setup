...
Если 128MB — это best-effort профиль, требующий активного swap от 384 МБ (512 МБ предпочтительно; `SwapTotal` в `/proc/meminfo`): проверь `free` (есть ли swap и сколько) или сразу `mihomo-doctor.sh` — он показывает объём, свободный остаток и бэкенды (zRAM отдельно от дискового). Учти: авто-размер zRAM ~равен физической RAM, сам по себе он требование 384 МБ на этом классе не закрывает. Не запускай второй Mihomo; при нестабильности переходи с `ram` на `disk`/отключай S00ubifs. Проект swap сам не создаёт. Для штатной эксплуатации — 256+ MB (swap опционален) или 512+ MB.
