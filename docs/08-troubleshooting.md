...
Если 128MB — это best-effort профиль, требующий активного swap от 384 МБ (512 МБ предпочтительно; `SwapTotal` в `/proc/meminfo`): проверь `free` (есть ли swap и сколько), свободную RAM, не запускай второй Mihomo, а при нестабильности перейди с `ram` на `disk`/отключи S00ubifs. Проект swap сам не создаёт. Для штатной эксплуатации — 256+ MB (swap опционален) или 512+ MB.
