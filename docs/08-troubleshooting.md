...
Если 128MB — это best-effort профиль, требующий внешнего /opt и внешнего storage-backed swap >= 384 МБ (512 предпочтительно; zRAM не считается): проверь `free` или сразу `mihomo-doctor.sh` — он показывает класс /opt, объём, свободный остаток и бэкенды (zRAM отдельно от внешнего дискового). Учти: авто-размер zRAM ~равен физической RAM, сам по себе он требование 384 МБ на этом классе не закрывает — нужен дисковый swap на внешнем носителе. Не запускай второй Mihomo; при нестабильности переходи с `ram` на `disk`/отключай S00ubifs. Проект swap и носители сам не создаёт/не монтирует. Для штатной эксплуатации — 256+ MB (внутренняя память — с активным zRAM; внешняя — swap опционален) или 512+ MB.
